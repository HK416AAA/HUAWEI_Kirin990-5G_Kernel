...
out:
	antiroot_debug("rm_mem_init successful!\n");
	return 0;
}
...
int initial_rm_shared_mem(TEEC_TempMemoryReference *rw_mem,
			TEEC_TempMemoryReference *swap_mem)
{
	if ((swap_mem == NULL) || (rw_mem == NULL)) {
		antiroot_error("Bad param!\n");
		return -EINVAL;
	}

	if (g_swap != NULL) {
		swap_mem->buffer = g_swap;
		swap_mem->size = RM_PRE_ALLOCATE_SIZE;
	} else {
	...
	if (g_rwmem != NULL) {
		rw_mem->buffer = g_rwmem;
		rw_mem->size = RM_PRE_ALLOCATE_SIZE;
	} else {
		antiroot_error("initial_rm_shared_mem failed!\n");
		return -ENOMEM;
	}
	antiroot_debug("initial_rm_shared_mem successful!\n");
	return 0;
}
