...
int tui_send_event(int event, struct teec_tui_parameter *tui_param)
{
	...
	if (g_dss_fd == NULL || tui_param == NULL)
		return -1;

	status_temp = atomic_read(&g_tui_state);
	check_value = (status_temp != TUI_STATE_UNUSED && g_dss_fd->panel_power_on) ||
		TUI_POLL_NOTCH == event || TUI_POLL_FOLD == event;
	if (check_value) {
		struct tc_ns_smc_cmd smc_cmd = { {0}, 0 };
		struct mb_cmd_pack *mb_pack = NULL;
		int ret = 0;
