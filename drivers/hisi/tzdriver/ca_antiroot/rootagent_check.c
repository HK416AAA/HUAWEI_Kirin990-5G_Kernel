...
static int tee_kcode_proc_show(struct seq_file *m, void *v)
{
	...
	ret = tee_status_check(KERNELCODEBIT);
	if (ret != 0)
		antiroot_error("tee kcode check failed, ret = %d\n", ret);
	else
		antiroot_debug("tee kcode check ok\n");
	return ret;
}
...
static int tee_sehooks_proc_show(struct seq_file *m, void *v)
{
	...
	ret = tee_status_check(SEHOOKBIT);
	if (ret != 0)
		antiroot_error("tee sehooks check failed, ret = %d\n", ret);
	else
		antiroot_debug("tee sehooks check ok\n");
	return ret;
}
...
static int tee_syscall_proc_show(struct seq_file *m, void *v)
{
	...
	ret = tee_status_check(SYSTEMCALLBIT);
	if (ret != 0)
		antiroot_error("tee syscall check failed, ret = %d\n", ret);
	else
		antiroot_debug("tee syscall check ok\n");
	return ret;
}
...
static int tee_kenerl_address_proc_show(struct seq_file *m, void *v)
{
	...
	ret = tee_status_check(CHECKFAILBIT);
	if (ret != 0)
		antiroot_error("tee no kenerl addr check failed , ret = %d\n",
			ret);
	else
		antiroot_debug("tee no kenerl addr check ok\n");
	return ret;
}
...
static int tee_rodata_proc_show(struct seq_file *m, void *v)
{
	...
	ret = tee_status_check(RODATABIT);
	if (ret != 0)
...
void handle_proc_create(void)
{
	...
	antiroot_debug("tee verify proc creat\n");
#endif // CONFIG_TEE_ANTIROOT_CLIENT_ENG_DEBUG
}
...
void handle_proc_create(void)
{
	 ...
	 * such as open, read
	 */
	proc_create_single("ts_tm_rodata", 0440, NULL, &tee_rodata_proc_show);
#endif

	/* 0440 is mean only user and user group have read premission */
	proc_create_single("ts_tm_kcode", 0440, NULL, &tee_kcode_proc_show);
	proc_create_single("ts_tm_syscall", 0440, NULL, &tee_syscall_proc_show);
	proc_create_single("ts_tm_sehooks", 0440, NULL, &tee_sehooks_proc_show);
	proc_create_single("ts_tm_kaddr", 0440, NULL, &tee_kenerl_address_proc_show);
	antiroot_debug("tee verify proc creat\n");
#endif // CONFIG_TEE_ANTIROOT_CLIENT_ENG_DEBUG
}
